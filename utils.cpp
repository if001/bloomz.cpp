...
struct bloom_sp_bigram {
    struct comparator {
        bool operator()(bloom_sp_bigram & l, bloom_sp_bigram & r) {
            // return (l.score < r.score) || (l.score == r.score && l.left > r.left);
            return (l.left > r.left);
        }
    };
    using queue_storage = std::vector<bloom_sp_bigram>;
    using queue = std::priority_queue<bloom_sp_bigram, queue_storage, comparator>;
    bloom_sp_symbol::index left;
    bloom_sp_symbol::index right;
    // float score;
    size_t size;
};
...
struct llama_tokenizer {
    ...
    void tokenize(const std::string & text, std::vector<gpt_vocab::id> & output) {
...
private:
    ...
    std::vector<bloom_sp_symbol> symbols_;
    bloom_sp_bigram::queue work_queue_;
};

std::vector<gpt_vocab::id> bloom_tokenize2(const gpt_vocab & vocab, const std::string & text, bool bos) {
